...
const VoiceCommandSheet = lazy(() => import('@/components/VoiceCommandSheet').then(m => ({ default: m.VoiceCommandSheet })));
...
export function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 animate-slide-up nav-safe-area">
      <div className="relative w-full max-w-lg mx-auto pb-2">

        {/* Glassmorphism Background Container */}
        <div className="relative mx-3 h-[68px] bg-card/90 backdrop-blur-2xl border border-white/20 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.12)] rounded-[2rem] overflow-hidden">
          ...
          {/* Nav Items Container */}
          <nav aria-label="Main navigation" className="relative h-full flex items-center justify-between px-2 z-20">
            ...
            <div className="flex items-center justify-between w-[42%] h-full pl-1">
              ...
              <NavItem
                item={navItems[1]}
                isActive={location.pathname.startsWith(navItems[1].path)}
                onClick={() => navigate(navItems[1].path)}
              />
              <NavItem
                item={navItems[2]}
                isActive={location.pathname.startsWith(navItems[2].path)}
            ...
            <div className="flex items-center justify-between w-[42%] h-full pr-1">
              <NavItem
                item={navItems[3]}
                isActive={location.pathname.startsWith(navItems[3].path)}
                onClick={() => navigate(navItems[3].path)}
              />
              <NavItem
                item={navItems[4]}
                isActive={location.pathname.startsWith(navItems[4].path)}
                onClick={() => navigate(navItems[4].path)}
              />
              <NavItem
                item={navItems[5]}
                isActive={location.pathname.startsWith(navItems[5].path)}
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/3 z-30 pointer-events-none">
            ...
            <Suspense fallback={null}>
              <VoiceCommandSheet>
                ...
                >
                  ...
                  <Mic className="w-7 h-7 text-primary-foreground drop-shadow-md transition-transform duration-500 group-hover:scale-110 group-active:scale-90" aria-hidden="true" />
                </Button>
              </VoiceCommandSheet>
            </Suspense>
          </div>
...
function NavItem({ item, isActive, onClick }: { item: NavItemConfig, isActive: boolean, onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      {isActive && (
        <span className="absolute inset-0 bg-primary/10 rounded-2xl animate-scale-in border border-primary/5" aria-hidden="true" />
      )}
      ...
      <div className={cn(
        ...
        <Icon
          className={cn(
            ...
            isActive && "fill-current drop-shadow-[0_2px_8px_rgba(var(--primary),0.3)] animate-wiggle"
          )}
        ...
        {/* Active Glow Dot */}
        {isActive && (
          <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-primary rounded-full shadow-[0_0_8px_currentColor] animate-fade-in" aria-hidden="true" />
        )}
      </div>

      {/* Label - fades in/out on selection — hidden from AT since aria-label covers it */}
      <span className={cn(
