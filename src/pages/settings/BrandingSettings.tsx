...
const DocumentPreview = ({ form }: { form: BrandingPreviewProps }) => {
...
const EmailPreview = ({ form }: { form: BrandingPreviewProps }) => (
      ...
      <div className="min-h-screen pb-20 scrollbar-hide">
        ...
        <div className="sticky top-0 z-30 glass border-b border-border/40 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60">
          <div className="px-4 py-3 flex items-center justify-between">
            ...
            <Button onClick={handleSubmit} disabled={saveBranding.isPending} size="sm" className="rounded-full shadow-glow-sm">
              {saveBranding.isPending ? <span className="animate-spin mr-2">⟳</span> : <Check className="w-4 h-4 mr-1" />}
              Save
            </Button>
          </div>
        </div>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            {['left', 'center', 'right'].map((pos) => (
                              <button
                                key={pos}
                                type="button"
                                onClick={() => setForm({ ...form, logo_position: pos as 'left' | 'center' | 'right' })}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {['gradient', 'solid', 'minimal'].map((style) => (
                          <div
                            key={style}
                            onClick={() => setForm({ ...form, document_header_style: style as 'gradient' | 'solid' | 'minimal' })}
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="sticky top-24 space-y-4">
              ...
              <div className="animate-fade-in transition-all duration-300 transform">
                {activeTab === 'emails' ? <EmailPreview form={form} /> : <DocumentPreview form={form} />}
              </div>
