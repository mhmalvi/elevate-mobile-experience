...
serve(async (req) => {
  ...
  try {
    ...
    const rateLimit = await checkRateLimit(supabase, user.id, 'send-team-invitation', 10, 60);
    if (rateLimit.limited) {
      return new Response(JSON.stringify({ error: 'Too many requests. Please wait before sending more invitations.' }), {
        status: 429,
    ...
    console.error('Fatal error in send-team-invitation:', errorMessage);
    return new Response(JSON.stringify({ error: "Failed to send team invitation" }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
